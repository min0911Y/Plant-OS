...
- 保持每 CPU 的 current、idle 和运行队列，运行队列及累计权重随状态、迁移和权重变化同步维护；选任务与选核不扫描任务注册表，外部阻塞统一走 task_fall_blocked_reason，权重修改走 task_set_weight。设计和基准见 [调度器](doc/scheduler.md)。只有 BSP 推进全局时钟及 timeout。调度器不可自切换；BSP 在资源就绪且释放最外层 kernel lock 后唤醒 AP，AP 等待 release 时休眠。
