...
- 全部正式用户应用均使用原生 `ET_DYN` PIE 和 `/lib/ld.so`，由用户态链接器选择装载地址；只有解释器自身保持静态自举（i386 `0x70000000`、x86_64 `0x100000000`）。C/C++ 统一由 `apps/libp/entry.c` 的 `Main` 初始化运行库并调用 `main(argc, argv)`，C++ 支持来自 `apps/libp/cxx.cpp` 构建的 `libcpp.so`。
...
- 同步 `os_execute` 在同一个 IRQ 临界区发布子进程并交出父进程 TTY；鼠标只在调用者本人持有时交接，返回时也只在没有其他 owner 时恢复调用者。不能保存并恢复其他任务的裸 mouse owner，也不能因进入命令前没有 owner 而清除命令执行期间启动的后台 GUI 所有权。
...
- 用户态 syscall 的编号和参数顺序唯一维护在 `apps/libp/arch/syscalls.inc`，两个架构用各自 CALL 宏展开。i386 必须保存 EBX（PIC GOT）、ESI、EDI、EBP，禁止恢复手写不配对的 push/pop 或独立重复的 VFS/socket 汇编包装。`__divmoddi4` 以无符号绝对值完成除法，再分别恢复商和余数的符号，不能用有符号移位循环实现；`timetest` 覆盖该编译器辅助 ABI 的边界值。
...
- x86_64 只接收 Limine native/base revision 3 的 HHDM、内存图、framebuffer、RSDP、MP 和 FAT module；四级页表根据内存图建立，不能探测写 RAM、扫描 BIOS RSDP 区或调用实模式服务。用户 ELF 必须是 ELF64/EM_X86_64；内核静态 loader 拒绝 PT_INTERP/PT_DYNAMIC、段重叠及 W+X 段。带 PT_INTERP 的程序由通用进程启动路径把原始可执行文件 fd 交给独立用户态解释器，动态链接不进入内核。
- x86_64 的 Limine MP request 必须设置 `LIMINE_MP_REQUEST_X86_64_X2APIC`，声明现有后端支持 x2APIC；否则固件锁定 x2APIC 时 Limine 会在进入内核前因无法降级而 PANIC。APIC 初始化以 `IA32_APIC_BASE` 的现有模式和 CPU 能力选择接口，已经启用的 x2APIC 不得退回 MMIO，也不依赖 LAPIC MMIO 映射。用 `scripts/test-x86_64.py --apic x2apic --firmware bios`/`--firmware uefi` 检查 Limine 实际交接模式和 SMP 运行，`--apic xapic` 检查无 x2APIC 能力的路径；QEMU 必须支持所请求的模式，不能忽略不支持特性的警告。
...
- x86_64 的 C/C++ 浮点 ABI 使用 SSE2，`long double` 为 binary64；两侧均使用 `-mno-red-zone -msse2 -mfpmath=sse -mlong-double-64`。BSP 检查 CPUID 的 XSAVE、x87/SSE 状态位和 XSAVEOPT 能力后，为持久任务上下文选择 XSAVEOPT64/XRSTOR64，否则使用 FXSAVE64/FXRSTOR64；AP 必须验证所选后端可用。只有选择 XSAVEOPT 时才设置 CR4.OSXSAVE，所有 CPU 的 XCR0 固定为 x87|SSE（3），不启用 AVX。任务状态为 64 字节对齐的 576-byte 标准 XSAVE 区，header 保留位初始化为零；恢复后不能清空或改写保存区再用 XSAVEOPT，当前任务 reset 必须在 IRQ 临界区同步恢复硬件，重置 modified tracking。临时中断/syscall frame 仍保存完整 512-byte FXSAVE64 状态，再装入内核 MXCSR，不能对清零或复用的栈区域直接使用 XSAVEOPT。调度、fork 和信号返回保留 16 个 XMM 寄存器、x87 及 MXCSR；信号 ABI 不包含 XSAVE header。不得在共享代码恢复 x87 内联数学汇编，i386 的 x87 后端仍独立保留。
...
- 旧 SB16 驱动没有活动的音频调用接口，已连同启动入口、任务清理引用和 QEMU 声卡参数删除。启动不得无条件占用、改写或解屏蔽 IRQ5；此前 `disable_sb16()` 实际将该线配置为低电平触发并开启，可能造成真机中断风暴，也会阻止 PCI INTx 共享注册。将来接入 ISA 音频时必须先检测设备，再按固件路由及设备语义配置 IRQ，并使用正式 DMA 内存接口和有界等待，禁止恢复固定 `0x90000` 指针及无限忙等。
...
- i386 的 `arch_mmio_map` 在分页后通过 page 后端建立并标记内核共享的设备/固件恒等映射；`pf_set` 不得按可用 RAM 上限清掉这些已映射页，否则后续读取 RAM 顶部保留区中的 ACPI/MCFG 会页故障。映射需要校验范围并拒绝覆盖当前地址空间的私有用户页表；只在页表实际改变时刷新本地 TLB，不能再把“返回物理地址指针”当作完成 MMIO 映射。
- PCI 配置访问优先使用校验过的 ACPI MCFG/ECAM，`pci_device_t` 保留 16-bit segment；地址按原始 MCFG bus-zero base 加 `bus<<20 | slot<<15 | function<<12 | offset` 计算，不得把非零 StartBus 重复减掉。按实际访问的总线映射并缓存 1 MiB 窗口，同时以映射记录避免重复枚举；MCFG 同 segment 的范围不得重叠。MCFG 描述配置空间覆盖范围，不是根桥列表：先枚举 segment 0/bus 0，再覆盖 MCFG 范围内尚未访问的总线，沿已配置桥的 secondary bus 递归；每条总线只访问一次，空 slot 只读 function 0，只有有效 multifunction 设备才读其余 function。不得恢复无固件范围依据的 256×32×8 穷举，也不能只从 StartBus 沿桥遍历而漏掉范围中间的独立根；不得猜测 Host Bridge 的 function 编号就是根总线编号。没有对应 ECAM 窗口时只允许 segment 0 使用 CF8/CFC；端口事务保持 IRQ 临界区，ECAM 按真实 16/32-bit 宽度访问。Vendor ID 为 0/ffff 不注册，PCIe CRS 的 0001 必须在 1 秒 deadline 内重读，不能把尚未就绪的功能缓存为正常设备。`--machine q35` 验证 MCFG 路径，`--machine pc` 验证无 MCFG 的端口路径。
- `pci_find_class(class, subclass, after)` 通过注册表游标遍历同类设备；`pci_read_bar` 同时取得地址与真实 aperture 大小，探测时关闭 decode/bus master 并恢复原 BAR/command。该探测只用于接管设备时，不能对正在传输的设备重复探测；配置地址/数据端口事务必须保持中断临界区。修改 command 使用 `pci_command_update`，不能用 32-bit 写操作误清相邻 PCI status 的 W1C 位。
- PCI 消息中断由 `pci_irq_initialize`/`pci_irq_release` 管理：设备静止后按 MSI-X、MSI、共享 INTx 顺序选择，PCI Interrupt Line 为 `255` 不得阻止消息中断接入。MSI 使用一个消息并处理 32/64-bit capability；MSI-X 校验 BIR、真实 BAR aperture 与完整 table 范围，先屏蔽全部条目再配置并只开启 primary interrupter 的 entry 0。配置 command、MSI/MSI-X control 和 MSI data 必须按 16-bit 写，不能覆盖相邻 capability header 或 W1C status；不能把 64-bit BAR 的高半部分当作独立 BAR。停止控制器后先关闭设备中断源、回读确认，再注销 handler；共享注册按引用计数释放，不能屏蔽仍由其他设备使用的线。
- 消息 IRQ 通过 `irq_allocate_message` 分配独占 handler 和架构生成的 address/data；x86 使用 DPL0 的 `0x40..0xef` 向量，避开有线 IRQ、i386 `int 0x36`、IPI 与 spurious。i386 复用标准 IRQ frame，x86_64 使用统一 IDT 入口；回调后的 EOI 仍由分派器统一发送。消息不得进入 IOAPIC mask/config 路径；目标为 BSP，xAPIC/x2APIC 均可使用标准 MSI，未实现 interrupt remapping 时必须拒绝无法用 8-bit physical destination 表示的 APIC ID，不能截断或广播。
- USB 后端是 `kernel/drivers/xhci.c`，公共描述符、接口绑定和工作队列位于 `kernel/include/usb.h`、`kernel/drivers/usb.c`，Hub、HID 与 BOT/SCSI 分别在 `usb_hub.c`、`usb_hid.c`、`usb_storage.c`；i386/x86_64 共用。xHCI 按 PCI `0c:03:30` 遍历控制器，以 Supported Protocol/PSI 识别端口和速度，不能假设端口顺序。当前支持 xHCI 1.x、MSI-X/MSI 与共享 INTx、控制/bulk/interrupt 端点、根端口与 USB 1.x/2.0 Hub 拔插、标准键盘与相对鼠标、BOT/SCSI 数据盘；只选择第一个 configuration 的 alternate setting 0。SuperSpeed Hub 分支、UAS、绝对坐标 HID、GPT/扩展分区及 exFAT/NTFS 尚未实现；不得把只打印 `enumerated` 当作类驱动可用。
- xHCI DMA 使用正式 page/DMA/MMIO API，遵守 AC64、32/64-byte context、scratchpad、64 KiB TRB 边界与 cycle ownership；完整控制请求构造完成后最后发布 Setup TRB。命令与每个端点分别保存完成状态，不能恢复控制器级的单一传输槽。EP0 缓冲按需增长，bulk 使用对齐的 64 KiB 缓冲；不对串行 BOT 的每个阶段增加固定 interrupt moderation 延迟。IRQ 消费事件、处理预解析的 HID 输入并重新提交 interrupt 请求；正常构建中纯 HID 流量只唤醒输入消费者，USB service task 只因命令/同步传输/端口/工作请求而唤醒。IRQ 不分配或阻塞，枚举、端点配置、SCSI 和描述符解析均在 service task 中完成。命令/控制等待上限 1 秒，bulk 上限 5 秒；超时先停控制器，不能回收仍可 DMA 的内存，未确认 halt 时禁用 bus master 并隔离 DMA 页。
- USB 根端口与 Hub 子端口统一使用 `usb_port_t`，设备由端口持有，子设备经同一个 xHCI 构造/释放路径管理；`usb_connected` 必须核对祖先，Hub 断开后立即停止向下游投递输入/传输，先释放所有子 slot 再释放 Hub slot。USB 2.0 Hub 类驱动按描述符分配端口、控制供电并等待 PowerGood；连接稳定 100ms 后复位，消抖/复位各限 1 秒。状态 interrupt 只标记变化并唤醒 service，GET_STATUS/CLEAR_FEATURE、枚举和回收在任务上下文执行；失败端口没有新变化时不得无限重试。xHCI Route String 按五级四位端口字段编码，大于 15 的端口按规范编码为 15；低/全速设备的 TT Slot/Port 来自最近的高速祖先及其实际下游端口，保留 single/multi-TT 与 Think Time，全速中间 Hub 不得丢失上游 TT 属性。Hub/端点配置输入 slot 中的设备地址和状态等只读字段必须清零。普通键鼠插入 USB 3.x 插口仍走 USB 2.0 信号，内置 Hub 的 USB 2.0 分支同样需要枚举，不能根据插口外观把键鼠按 SuperSpeed 初始化。
- USB 真机修复已由用户确认可用，包括 270KP 原先失效的 USB 3.x 插口键鼠路径；日常构建与交付保持 `USB_DEBUG=0`（默认），关闭屏幕 USB 日志、端口转储及 HID 报告追踪，串口保留生命周期与错误信息。诊断模式只通过显式 `USB_DEBUG=1` 开启，不能把诊断开关写死进驱动；正常 x86_64 交付镜像为 `kernel/plant-os-x86_64.iso`，生成的 ISO 不进入源码提交。
- HID 按 Application Collection、Report ID、Usage、Report Size/Count 及有符号范围解析 Input 报告，拒绝截断描述符和越界长度；键盘处理修饰键、数组/NKRO 输入和 rollover，按下修饰键先于普通按键投递。PS/2 与 USB 共用 `kernel/io/input_device.c`：按设备维护键状态并合并引用，USB 重复输入由已有 timer tick 驱动，拔出时释放按键和鼠标按钮；不得把 USB 报告伪装成 PS/2 扫描码字节流再解析。逻辑键码沿用活跃 ABI 的 Set 1 编号及扩展标记。
- 鼠标用户 ABI 是 `mouse_read(mouse_event_t *)`/`SYSCALL_MOUSE_DATA`（`0x4f`），一次读取含 x/y/wheel/buttons 的 16-byte 事件，返回 1/0/负错误；内核与 `apps/include/input_event.h` 必须保持布局一致，两种汇编包装完整传递指针。完整事件的写入和读取均使用 IRQ 临界区，FIFO 不允许半条事件或交错的扩展键前缀，GUI 直接消费相对移动与滚轮，禁止恢复 `mouse_dat_get`、用户态 PS/2 decoder 或全局 `mdec`；坐标累加先提升到 64 位再裁剪。
- BOT/SCSI 工作按 USB service task 串行执行，CBW/CSW 必须验证长度、签名、tag、residue 和状态；STALL 经端点恢复处理，传输失败不盲目重放写入。READ CAPACITY(10/16)、READ/WRITE(10/16) 保留 64 位容量/LBA，较大原生块经读改写适配 512-byte 磁盘 API；支持无分区介质和 MBR 主分区，挂载复用既有 FAT/PFS。USB 请求持有独立缓冲和类驱动引用，任务回收取消未完成的外部请求，不能把 DMA 指向等待调用者的栈或用户地址。
- AHCI 使用 `kernel/drivers/ahci.c` 的控制器/端口状态和统一 IDENTIFY、READ/WRITE DMA、FLUSH 命令路径；每端口持有 page-backed 命令/FIS 区与 64 KiB DMA 缓冲，不向调用者栈或用户内存 DMA，不保留旧的共享全局 cache 和三份重复传输循环。使用 PCI MSI-X/MSI/INTx 完成中断及 `WAIT_REASON_DISK`/一次性 timer 阻塞，等待者以 TID/generation 识别。固件交接最多 2 秒，控制器复位、CR/FR 停机与设备就绪等待各最多 1 秒，命令最多 5 秒；必须先清 ST 并确认 CR=0，再清 FRE 并确认 FR=0。失败不重放写入；停机未确认时禁用 bus master 并隔离 DMA 页，不能超时返回后释放仍可能被写入的内存。初始化失败打印控制器/端口寄存器并继续启动。容量按 IDENTIFY 的 LBA28/48 字段解析，fsync 下传缓存刷新；当前只支持 512-byte 逻辑扇区的 SATA 磁盘，ATAPI、端口倍增器及 SATA 热插拔明确不支持。
...
- 动态链接器唯一实现位于 `apps/ldso/`，安装为 `/lib/ld.so`。它自身是静态原生 ELF；内核只解析 `PT_INTERP` 并用 `loader_start_t` 交付已打开的原始 ELF fd、实际路径及解释器带盘符路径，命令行保持原样。解释器从系统启动盘打开，用户链接器以其所在目录作为默认库目录，跨盘执行不能错误地到调用者当前盘查找系统运行库。禁止把 DT_NEEDED 搜索、动态符号解析或重定位移进内核。ELF 格式与启动 ABI 分别由 `apps/include/elf.h`、`loader.h` 单源定义，内核头文件只转引。
- 动态程序与共享库使用 ET_DYN/PIC，i386 和 x86_64 分别保留原生 REL/RELA 语义；PLT 在启动时全部解析，支持 SysV/GNU hash、弱符号、符号覆盖、RUNPATH/RPATH 与 `$ORIGIN`。只含导入符号的空 GNU hash 不能把 `symoffset=1` 当作 dynsym 总条目数。所有表和重定位目标必须落在已验证的 LOAD 内，禁止文本重定位、段/页重叠或 W+X；GNU RELRO 的尾部允许延伸到 LOAD 页内填充，按页面覆盖范围校验。
- `libp` 完成 allocator、stdio、参数和环境初始化后才经 `runtime_linker_t` 调用动态构造函数，依赖先于使用者，析构按实际初始化顺序逆序执行。链接器元数据使用独立 VM arena，不能调用应用分配器再让应用入口重置它。依赖图必须去重并有界处理循环，不依赖递归深度或固定库数量。
- `SYSCALL_VM`（`0x66`）提供匿名私有 map/protect/unmap；指定地址不能覆盖现有映射，heap 增长必须检查与高端映射冲突，失败回滚。i386 `PG_COW` 与真正只读权限必须分开，用户映射及其页表只由地址空间引用持有，不能在创建线程退出时被 GC 提前回收。x86_64 执行 W^X/NX 与 RELRO，非 PAE i386 保证写保护但没有 NX。
- 公共 C/C++ 运行库分别为 `libp.so`、`libcpp.so`，位于 `apps/out[/x86_64]/lib/`；所有应用和应用私有归档都使用 PIC，链接带 `-pie`、`-z text`。i386 内核的非 PIC 编译器辅助归档与用户态 PIC libtcc1 必须分开；TCC SDK 的静态归档只服务编译器，不得重新链接进正常应用。
- i386 TCC SDK 在 `out/sdk/` 单独编译非 PIC 对象，不得复用动态运行库的 PIC 归档；TCC 自身仍为 PIE。TCC 预定义目标位宽整数类型、指针宽度和 i386 cdecl 可变参数内建名，ABI 静态断言必须真正检查表达式；共享头文件不能依赖缺失的编译器私有类型。`--tools --arch i386` 验证 SDK 编译、链接和运行普通 C 程序。
- 当前未实现动态 ET_EXEC、TLS、IFUNC、符号版本、RELR、运行期 dlopen/dlsym/dlclose 及 LD_PRELOAD/LD_LIBRARY_PATH，不得伪装支持。构建与 ABI 说明见 `doc/dynamic-linking.md`；回归使用 `scripts/test-x86_64.py --dynamic` 和 `--arch i386 --dynamic --memory 512`，默认完整回归也执行 dyntest；`--all-apps` 依据构建清单逐项核对 ELF 并通过 `/lib/ld.so --verify` 完成真实装载与重定位。FAT 路径从镜像中的 `apps.lst` 取得，不能猜测长文件名别名。测试应覆盖跨库调用与失败路径，并检查真正的只读/RELRO 写故障、fork 和构造/析构顺序；继续通过临时 init.mst 注入命令并恢复镜像。

- shell 命令和普通程序均使用 `os_execute`，禁止恢复 `shell_data`、`task_shell` 或静态 shell 直装路径。`EXECUTE_COMMAND` 在正常退出时通过任务内部 `return_cwd` 返回 cwd，普通执行与 fork 不继承这一权限；禁止恢复用户可写的退出状态页。系统盘探测必须同时验证 `/lib/ld.so` 和 `/lib/libp.so`。

## 构建环境与命令

需要 GNU make、支持 `-m32`/`elf_i386` 的 GCC/G++ 与 binutils、NASM、mtools（`mformat`、`mcopy`）和 QEMU（`qemu-system-i386`、`qemu-img`）。LiveCD 还需要 `curl`、`tar` 以及 `xorriso` 或 `genisoimage`。编译器必须同时支持内核的 freestanding 非 PIE 输出和用户态 PIC/PIE 输出。
...
- SIMD 后端回归通过 `--cpu` 指定 QEMU CPU 特性：默认 `max` 验证 XSAVEOPT，`max,xsaveopt=off,enforce` 验证只有 XSAVE 时的 FXSAVE fallback，`max,xsave=off,xsaveopt=off,xsavec=off,xsaves=off,enforce` 验证完全没有 XSAVE 的 CPU。检查串口 `simd: context=... xcr0=...` 与 `SIMDTEST PASS`；`simdtest.bin` 覆盖多核让出、fork、16 个 XMM、x87 栈/control/status/tag、MXCSR 及显式 reset 后的初始状态。可以使用 `--accel kvm --cpu host,enforce` 在支持 XSAVEOPT 的宿主上验证真实指令的 modified optimization，不能把 TCG 模拟等同于真机优化路径。
- USB 回归运行 `scripts/test-x86_64.py --usb --firmware bios`、`--usb --firmware uefi --memory 6144` 和 `--arch i386 --usb --memory 512`。QEMU 禁用 i8042，使用两个 xHCI、USB 键鼠及三个临时 FAT 介质（MBR、无分区、4 KiB 原生块）；`usbtest.bin` 校验读写、fsync、非对齐写入、拔插及旧句柄隔离，退出 QEMU 后再用 mtools 独立检查实际文件。`guitest.bin usb` 验证精确鼠标坐标/按键/滚轮、键盘修饰键/方向键/重复及拔出释放，再检查 shell 回显、退格、滚屏和 100 次键盘拔插。USB 1.0 audio 仅用于 EP0 包大小回归；音频不受支持。命令与配置通过临时 `init.mst`/`sys.cfg` 注入并恢复，QMP 按键只验证输入，禁止执行命令。
- `--usb --usb-hubs` 将键鼠接到支持 USB2/USB3 双协议的 xHCI 的 USB 2.0 端口下，两级 Hub 均启用端口供电控制；检查真实键鼠事件、100 次子端口拔插、根 Hub 连同所有后代移除/重建及 route。可组合 `--usb-root-bus`/`--usb-no-intx`，命令仍经临时 init.mst 注入。QEMU 当前 Hub 只模拟 full-speed，不能据此宣称真机高速 Hub 的 split transaction 或 SuperSpeed Hub 已验证；TT 字段另做宿主测试，实际链路仍需真机确认。
- AHCI 回归使用 `scripts/test-x86_64.py --ahci --machine q35`，可组合 `--firmware uefi` 或 `--arch i386 --memory 512`；两个临时 SATA 盘验证预置文件读取、写入、非对齐覆盖、fsync 和 160 GiB 稀疏盘的 LBA48 容量，退出 QEMU 后用 mtools 独立核对实际文件。`--ahci-no-irq` 在第一条 IDENTIFY 前通过 QEMU GDB/qtest 清除 GHC.IE，必须观察命令超时、控制器初始化结束及进入 shell，不能用轮询 CI 当作丢失中断的成功兜底。命令仍仅通过临时 init.mst 注入，测试后恢复。
- 独立 PCIe 根总线回归使用 `--machine q35 --usb-root-bus 0x80`，将两个 xHCI 放在 `pxb-pcie` 根下的两个 root port 后；它们没有从 bus 0 经 PCI-to-PCI bridge 可达的父路径。必须检查内核确实发现新入口总线和其下两个控制器，并完成键鼠、存储、拔插回归；可组合 `--usb-no-intx` 验证这些控制器的 IRQ=255。根总线号由参数指定，驱动不能硬编码 bus 0x80。
- USB 中断回归使用 `--usb-irq msix|msi|intx` 限制 QEMU capability 并检查实际选中模式；默认 `auto` 验证 MSI-X 优先。`--usb-no-intx` 通过 QEMU GDB 硬件断点在 `xhci_initialize` 入口暂停，再用 qtest 将两个控制器真实 PCI Interrupt Line 字节改为 `255` 并读回，恢复运行后必须经消息 IRQ 完成键鼠、存储和拔插测试；不得用内核假返回、轮询传输或按键执行命令替代这一复现条件。INTx 回归仍需单独运行以覆盖与网卡共享 IRQ 的路径。
- 无串口的真机排查使用 `make -C kernel ARCH=x86_64 USB_DEBUG=1 livecd`；两种架构均支持该开关，构建指纹必须跟踪它。USB 生命周期与失败原因统一通过 `usb_log` 写串口，诊断构建同时写屏幕并保留进入 shell 前的启动画面。日志需区分 PCI INTx 无效、固件接管、控制器复位、IRQ 命令自检、端口/描述符、Hub 供电/状态、下游 route/TT 及 HID 绑定；不得把没有可用中断路由伪装成键盘已就绪。诊断构建每个 HID 接口最多记录前 8 次报告的长度、ID、解析有效性、按键变化数量与调度唤醒结果，不输出字符、按键值或原始报告；IRQ 仅向固定队列复制摘要并唤醒 USB service task，由后者打印，正常构建不包含该队列与额外输入唤醒。`scripts/test-x86_64.py --usb --usb-debug` 验证此配置，测试恢复镜像时必须保留所选诊断开关。
...
- 驱动通过 `irq_register_handler(irq, handler, IRQ_EXCLUSIVE/IRQ_SHARED)`（`kernel/dos/hal/interrupt.c`）注册 IRQ 处理函数；ISA 使用 exclusive，PCI INTx 使用 shared，只有共享注册可以共用一条线。回调签名为 `bool handler(unsigned irq)`，仅确认本设备状态并返回是否需要调度；分派器遍历整条链后统一发送一次 EOI，再按返回值调度。回调及其调用链不得自行 EOI 或 `task_next()`。第一条注册不依赖堆，以支持早期 PIT/PS2；共享链按需扩容。DPL0 gate 仍由架构 IDT 初始化统一创建，DPL3 gate 只允许既有系统调用向量。PCI 驱动取得 `uint8_t` IRQ 后必须先调用 `irq_is_valid`，成功后才能配置路由或解屏蔽；底层 mask/config API 对非法 IRQ 安全返回。
...
- 正常应用不使用 `-N`、`-static` 或固定 `-Ttext` 地址；GNU ld 生成原生 PIE 的独立 R/RX/RW LOAD，入口必须位于 RX 段。固定地址静态 loader 只服务解释器自举和独立编译器 SDK 产物。
...
- `gui.bin` 是严格单例：必须在读取大资源、切换 VBE 或创建 `gmouse` 输入线程之前先注册 `gui` RPC 服务，服务名冲突时立即退出，禁止第二实例先覆盖显示模式或输入 owner。内核的 `mouse_enable()`/`use_keyboard()` 采用不可抢占的独占获取，已有其他任务持有时返回失败；GUI 输入线程必须检查返回值，不能静默抢占公共输入所有权。
...
- 正式应用子目录 Makefile 只指定 `NATIVE_TARGETS` 并包含 `../native.mk`；两种架构都转到 `apps/build.mk`。新增程序在 `apps/native-apps.mk` 的统一 `application` 宏中注册，不复制本地编译、链接与源码列表。
...
- 新应用必须进入公共构建图；`applications.list` 自动记录全部应用产物，不能手工维护另一份列表。
- 非 LiveCD 镜像仍由 `kernel/Makefile` 的显式 `mcopy` 控制；LiveCD 则按构建图生成的 `applications.list` 收录，不能扫描陈旧 `.bin`，也不能手工维护第二份应用列表。
- C/C++ 应用实现常规的 `main`，不要绕过 `apps/libp` 的 `Main` 启动包装，除非任务明确要求自定义运行时。
- 修改用户态库时检查 `libp.so`、`libcpp.so`、静态自举解释器和 i386 TCC SDK；C 与 C++ 共用同一 `entry.c`，不能恢复重复的 `cppstart.cpp` 初始化代码。

- i386/x86_64 用户程序构建统一由 `apps/build.mk` 提供公共规则，`apps/native-apps.mk` 维护应用、库与专属编译参数；单应用通过 `native.mk` 转到同一依赖图。SDL、lite、Doom、NASM 的 `sources.mk` 同时供两种架构使用，不能维护两份不同的源码清单，也不能把第三方对象写回源目录再被另一架构复用。动态链接器、共享运行库及回归程序由两种架构共用的 `apps/dynamic.mk` 构建；TCC 工具链、DOSLDR 安装器和 x87 专用 fputest 仍只构建 i386。
...
- `vdisk` 容量为 64 位字节数，Read/Write/Sync 返回真实成功状态；`disk_read`/`disk_write` 统一分批、检查范围并传递失败，VFS 在后端操作结束后核对设备健康状态，禁止把失败的 I/O 缓存或报告为成功。USB 驱动自己串行执行工作，声明 `owns_serialization`，不能再套一层 DriveSemaphore 让 service task 与等待它的调用者互相阻塞。物理盘注销会摘除挂载并失效缓存；仍有 retired 引用或 format reservation 的盘符不能分配给新设备，旧文件句柄不得访问重新插入的介质。`vfs_sync` 同时下传设备同步操作。
...
- 内核通用堆由 `kernel/dos/mm/heap.c` 统一封装按架构选择的 vendored liballoc/talc；x86_64 通用堆满足 16 字节对齐，任务注册表分块使用 `page_malloc/page_free` 以满足任务 XSAVE 状态的 64 字节对齐，不能把普通 `malloc` 当作过度对齐分配器；第三方导出在构建副本中统一改名为私有 `liballoc_*`，内核和模块只能使用带 KASAN/元数据校验的正式堆或 page API。heap 从 1 MiB span 起步并按需通过 `page_malloc` 增长，不得恢复 `memory/freeinfo` 排序整理器或启动时预清零固定 128 MiB arena。链接时 liballoc archive 必须紧跟 `heap.o`，使其静态锁状态位于固定 `0x400000` 页表区之前；放到对象列表末尾会在 KASAN 构建中被页表初始化覆盖。
...
- 启动系统盘必须同时包含 `init.bin`、`psh.bin`、`sys.cfg`、`lib/ld.so` 和 `lib/libp.so`；探测失败必须 panic，不得退回可能是 DEVFS 的 `first_vdisk()`。显式盘符剥离依赖标准重叠 `memmove` 语义，修改基础内存函数后必须做冷启动验证。
...
- 内核编译禁用宿主头文件、内建函数、栈保护和 PIE；i386 禁用 MMX/SSE 并使用 x87，x86_64 使用 SSE2。C++ 禁用异常和 RTTI。内核与静态程序必须保持这些限制，不要引入依赖异常、线程局部存储或宿主运行时的库；动态用户程序与共享库单独使用 `apps/dynamic.mk` 的原生 PIC 参数。
