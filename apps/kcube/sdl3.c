...
ASTATIC AWindow *aOpenWin(AInt16 x, AInt16 y, const char *t, AInt8 autoClose)
{
	...
	aInitWin0(w, x, y, t, autoClose);
        ...
        if (!w->txtr) {
          ...
          aWinTable_w[w->table_i] = NULL;
          aFree(w->buf, w->xsiz * w->ysiz * aSizeof(AInt32));
          aFree(w, aSizeof(AWindow));
          aErrExit("SDL3 window creation failed: %s", SDL_GetError());
          return NULL;
        }
...
ASTATIC void aFlushAll0(AWindow *w)
{
	...
	int linebyte;
        if (w->phase <= 1 && w->win != 0 &&
            SDL_LockTexture(w->txtr, 0, (void *)&q, &linebyte)) {
          ...
          SDL_RenderTexture(w->rndr, w->txtr, 0, 0);
	...
	do {
		...
		while (SDL_PollEvent(&ev) != 0) {
                  if (ev.type >= SDL_EVENT_WINDOW_FIRST &&
                    ...
                    int i;
                    static int tbl[] = {SDL_EVENT_WINDOW_SHOWN,
                    ...
                    if (ev.type == SDL_EVENT_WINDOW_MOUSE_ENTER) {
                      AWindow *w = aGetWinFromWId(ev.window.windowID);
                  ...
                  if (ev.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
                    ...
                    if (w != 0 && w->mosLv >= 1 && aGetSpcKeybuf(w) >= 2) {
                      ...
                      if (i >= 0) {
                        ...
                        aPutKeybuf(w, ((uint32_t)(int)ev.button.x & 0xffff) |
                                          ((uint32_t)(int)ev.button.y & 0xffff)
                                              << 16);
                      }
                    }
                  }
                  ...
                  if (ev.type == SDL_EVENT_MOUSE_MOTION) {
                    ...
                    if (w != 0 && w->mosLv >= 2 && aGetSpcKeybuf(w) >= 2) {
                      aPutKeybuf(w, 0x4020);
                      aPutKeybuf(w, ((uint32_t)(int)ev.motion.x & 0xffff) |
                                        ((uint32_t)(int)ev.motion.y & 0xffff)
                                            << 16);
                    }
                  }
#endif
