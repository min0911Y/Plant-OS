...
int main(int argc, char *argv[]) {
  ...
  SDL_Window *window = SDL_CreateWindow("Minesweeper", 16 * COUNT_OF_BLOCKS,
                                        16 * COUNT_OF_BLOCKS, 0);
  if (!window) {
    SDL_Quit();
    return 1;
  }
  ...
  if (!surface || !png_block || !png_mine || !png_flag) {
    SDL_Log("Minesweeper: %s", SDL_GetError());
    SDL_DestroySurface(png_block);
    SDL_DestroySurface(png_mine);
    SDL_DestroySurface(png_flag);
  ...
  while (!quit) {
	...
	while (SDL_PollEvent(&event)) {	// 消息循环
          if (event.type == SDL_EVENT_QUIT ||
              event.type == SDL_EVENT_WINDOW_CLOSE_REQUESTED) { // 按下关闭窗口
            quit = true;
          } else if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN &&
                     !end) { // 按下鼠标且游戏没结束
            if (event.button.x < 0 || event.button.y < 0 ||
                event.button.x >= surface->w || event.button.y >= surface->h)
              continue;
            int x = event.button.x / 16;
            int y = event.button.y / 16;
            if (x < 0 || y < 0 || x >= COUNT_OF_BLOCKS || y >= COUNT_OF_BLOCKS)
              continue;
            if (event.button.button == SDL_BUTTON_LEFT) { // 按下鼠标左键
              if (block_type[y][x] == 0) {
                ...
                sprintf(
                    name, "pngs/%d.png",
                    how_many_mines_around(x, y)); // 根据四周雷的个数来确定素材
                ...
                if (how_many_mines_around(x, y) == 0) {
                  ...
                  while (
                      stk.top !=
                      0) { // 这么写是因为有标记以扫过的区域 所以不会扫出死循环
                    int flag =
                        pop(&stk); // FLAG用于标记不用重复扫的区域（速度更快）
                    y = pop(&stk);
                    x = pop(&stk);
                    if (flag != 1) { // 横扫的两个循环
                      for (int i = x + 1; i <= COUNT_OF_BLOCKS; i++) {
                        if (block_type[y][i] == 0 &&
                            how_many_mines_around(i, y) == 0) {
                          block_type[y][i] = -1;
