...
bool now_tty_GraphicMode(struct tty *res) {
  ...
  if (console->window->sht->height == console->window->sht->ctl->top - 1) { return true; }
  return false;
}
...
void clear_console(struct tty *res) {
  ...
  res->x             = 0;
  res->y             = 0;
  console->window->draw(console->window, console->x, console->y, console->x + console->xsize,
...
static void copy_char(vram_t *vram, int off_x, int off_y, int x, int y, int x1, int y1, int xsize) {
  for (int i = 0; i < 16; i++) {
    for (int j = 0; j < 8; j++) {
...
void screen_ne_console(struct tty *res) {
  ...
  console->window->draw(console->window, console->x, console->y + (res->ysize - 1) * 16,
                        console->x + res->xsize * 8, console->y + res->ysize * 16,
                        text_color_to_real_color(res->color, false));
  res->x = 0;
  res->y = res->ysize - 1;
  sheet_refresh(console->sht_copy, console->x, console->y, console->x + console->xsize,
                console->y + console->ysize);
  sheet_refresh(console->window->sht, console->x, console->y, console->x + console->xsize,
                console->y + console->ysize);
  res->MoveCursor(res, res->x, res->y);
}
void Draw_Box_console(struct tty *res, int x, int y, int x1, int y1, unsigned char color) {
  ...
  sheet_refresh(console->sht_copy, console->x + x * 8, console->y + y * 16, console->x + x1 * 8,
                console->y + y1 * 16);
  sheet_refresh(console->window->sht, console->x + x * 8, console->y + y * 16, console->x + x1 * 8,
                console->y + y1 * 16);
}
...
struct tty *mtty_alloc(void *vram, int xsize, int ysize, void (*putchar)(struct tty *res, int c),
...
void mtty_handle(uint32_t *a) {
  ...
  switch (a[0]) {
  case 0: putchar_console(tty, a[2]); break;
  case 1: MoveCursor_console(tty, a[2], a[3]); break;
  case 2: clear_console(tty); break;
  case 3: screen_ne_console(tty); break;
  case 4: Draw_Box_console(tty, a[2], a[3], a[4], a[5], a[6]); break;
  case 5: a[0] = fifo8_status(cons->window->fifo_keypress); break;
  case 6: a[0] = fifo8_get(cons->window->fifo_keypress); break;
  default: break;
  }
  a[7] = tty->x;
  a[8] = tty->y;
...
console_t *create_console(window_t *window, int xsize, int ysize, int x, int y) {
  if (window->super_window != NULL) { return (console_t *)NULL; }
  ...
  res->tty     = mtty_alloc((void *)res, xsize / 8, ysize / 16, putchar_console, MoveCursor_console,
