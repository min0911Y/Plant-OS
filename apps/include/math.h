...
static __inline int __fpclassifyf(float x)
{
	uint32_t bits = __FLOAT_BITS(x);
...
static __inline __attribute__((unused)) int signbit(double x)
{
	union {
		double d;
		uint64_t i;
...
/* Standard scalar interfaces; availability is checked at link time. */
