...
/*
 * 有些朋友告诉我，我必须在一些函数的结尾加上const限定符，才能支持
...
class String {
	public:
		...
		char* getstr() const ;	//返回str，为了内存安全，建议使用c_arr，如果你只需要一个只读用的char*字符串，getstr函数足矣

		char* c_arr() const ;	   //将把String转换成char*类型并返回，返回值是一个存放于堆的char*指针，建议及时free

		bool match_head(String s) const ; //从头开始匹配s，匹配成功返回true，否则返回false

		String substring(int start, int end) ; //获取从start到end（不包含end）的子字符串
