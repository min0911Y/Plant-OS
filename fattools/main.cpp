...
void                 file_savefile(int clustno, int size, char *buf, int *fat, unsigned char *ff);
...
struct List *FindForCount(int count, struct List *Obj) {
  ...
  if (count > count_last) return (List *)NULL;
...
static inline int get_fat_date(unsigned short year, unsigned short month, unsigned short day) {
  year                -= 1980;
...
void Copy_out(char *path, char *path1) {
  ...
  time(&timep);
  p                   = gmtime(&timep);
  finfo1->update_date = get_fat_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
...
void file_loadfile(int clustno, int size, char *buf, int *fat) {
  ...
  for (int i = 0; i != (size - 1) / ClustnoBytes + 1; i++) {
    uint32_t sec = (FileDataAddress + (clustno - 2) * ClustnoBytes) / SectorBytes;
...
void file_savefile(int clustno, int size, char *buf, int *fat, unsigned char *ff) {
  ...
  clustno         = tmp;
  int alloc_size;
  if (size > (clustall + 1) * ClustnoBytes) { // 新大小 > (旧大小 / 簇大小 + 1) * 簇大小
    // 请求内存大小 = (新大小 / 簇大小 + 1) * 簇大小
    alloc_size = ((size - 1) / ClustnoBytes + 1) * ClustnoBytes;
    // 分配Fat（这里需要在写盘前分配）
    for (int size1 = size; size1 > ((clustall + 1) * ClustnoBytes); size1 -= ClustnoBytes) {
    ...
    ff[old_clustno]  = true;
  } else if (size <= (clustall + 1) * ClustnoBytes) { // 新大小 <= (旧大小 / 簇大小
                                                      // + 1) * 簇大小
  ...
  if (size < clustall * ClustnoBytes) { // 新大小 < (旧大小 / 簇大小) * 簇大小
    // 分配Fat（中间情况没必要分配）
    int i = old_clustno;
    for (int size1 = clustall * ClustnoBytes; size1 > size; size1 -= ClustnoBytes) {
      fat[i] = 0;
      ff[i]  = false;
...
void file_saveinfo(struct FAT_FILEINFO *directory) {
  if (directory == root_directory) {
    Disk_Write(RootDictAddress / SectorBytes, RootMaxFiles * 32 / SectorBytes, (void *)directory);
  } else {
    for (int i = 1; FindForCount(i, directory_list) != NULL; i++) {
      ...
      if ((struct FAT_FILEINFO *)list->val == directory) {
        ...
        Disk_Write((FileDataAddress + (list->val - 2) * ClustnoBytes) / SectorBytes,
...
void file_savefat(int *fat, int clustno, int length) {
  ...
  Disk_Write((Fat1Address + sec) / SectorBytes, size / SectorBytes + 1, ADR_DISKIMG + Fat1Address);
  Disk_Write((Fat2Address + sec) / SectorBytes, size / SectorBytes + 1, ADR_DISKIMG + Fat1Address);
  ...
  while (1) {
    ...
    if (finfo == 0) {
      ...
      if (get_clustno(finfo->clustno_high, finfo->clustno_low) != 0) {
        for (int count = 1; FindForCount(count, directory_clustno_list) != NULL; count++) {
...
void mkdir(char *dictname, int last_clust) {
  ...
  */
  ...
  time(&timep);
  p                    = gmtime(&timep);
  model[2].update_date = get_fat_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
  ...
  model[0].size               = 0;
  model[0].update_date        = get_fat_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
  ...
  model[1].size        = 0;
  model[1].update_date = get_fat_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
  ...
  finfo->size                = 0;
  AddVal(get_clustno(finfo->clustno_high, finfo->clustno_low), directory_clustno_list);
  // for (int count = 1;
  //      FindForCount(count,
  //                    directory_clustno_list) !=
  //      NULL;
  ...
  file_savefat(fat, get_clustno(model[2].clustno_high, model[2].clustno_low), 1);
  void *directory_alloc = _malloc(ClustnoBytes);
  Disk_Read((FileDataAddress +
             (get_clustno(finfo->clustno_high, finfo->clustno_low) - 2) * ClustnoBytes) /
                SectorBytes,
...
int del(char *cmdline) {
  ...
  file_savefile(get_clustno(finfo->clustno_high, finfo->clustno_low), 0, NULL, fat,
                FatClustnoFlags);
  finfo->name[0]                                                        = 0xe5;
...
int deldir(char *path) {
  ...
  if (finfo == Get_dictaddr(".")) { return 0; }
  ...
  if (finfo[1].clustno_low == 0) {
    ...
    for (int i = 1; FindForCount(i, directory_clustno_list) != NULL; i++) {
      if (FindForCount(i, directory_clustno_list)->val == finfo[1].clustno_low) {
        root_finfo = (struct FAT_FILEINFO *)FindForCount(i, directory_list)->val;
...
void mkfile(char *name) {
  ...
  time(&timep);
  p                  = gmtime(&timep);
  finfo->update_date = get_fat_date(p->tm_year + 1900, p->tm_mon + 1, p->tm_mday);
...
int format(char *mbr_path, int fat_type) {
  ...
  if (fat_type == 12 && (memcmp(read_in + BS_FileSysType, "FAT12   ", 8) == 0 ||
    ...
    *(unsigned short *)(&((unsigned char *)read_in)[BPB_TotSec16])   = imgTotalSize / 512;
    ...
    *(unsigned char *)(&((unsigned char *)read_in)[BS_DrvNum])    = 0;
    memcpy((void *)&((unsigned char *)read_in)[BS_FileSysType], (void *)"FAT12   ", 8);
    *(unsigned int *)(&((unsigned char *)read_in)[BS_VolD]) = 114514;
    memcpy((void *)&((unsigned char *)read_in)[BS_VolLab], (void *)"POWERINTDOS", 11);
    ...
    free(null_sec);
    // free((void*)info);
  } else if (fat_type == 16 && (memcmp(read_in + BS_FileSysType, "FAT12   ", 8) == 0 ||
                                memcmp(read_in + BS_FileSysType, "FAT16   ", 8) == 0)) {
    unsigned int clustno_size = ((imgTotalSize - 1) / 65536 + 512) / 512 * 512;
    *(unsigned char *)(&((unsigned char *)read_in)[BPB_SecPerClus])  = clustno_size / 512;
    *(unsigned short *)(&((unsigned char *)read_in)[BPB_RootEntCnt]) = 14 * clustno_size / 32;
    if (imgTotalSize / 512 > 65535) {
      *(unsigned short *)(&((unsigned char *)read_in)[BPB_TotSec16]) = 0;
    } else {
      *(unsigned short *)(&((unsigned char *)read_in)[BPB_TotSec16]) = imgTotalSize / 512;
    }
    *(unsigned int *)(&((unsigned char *)read_in)[BPB_TotSec32]) = imgTotalSize / 512;
    unsigned short fatsz = (imgTotalSize - 1) / clustno_size * 2 / 512 + 1;
    *(unsigned short *)(&((unsigned char *)read_in)[BPB_FATSz16]) = fatsz;
    *(unsigned char *)(&((unsigned char *)read_in)[BS_DrvNum])    = 0;
    memcpy((void *)&((unsigned char *)read_in)[BS_FileSysType], (void *)"FAT16   ", 8);
    *(unsigned int *)(&((unsigned char *)read_in)[BS_VolD]) = rand();
    ...
    free(null_sec);
             ...
             memcmp(read_in + BS_FileSysType + BPB_Fat32ExtByts, "FAT32   ", 8) == 0) {
    ...
    *(unsigned char *)(&((unsigned char *)read_in)[BS_DrvNum + BPB_Fat32ExtByts])    = 0;
    *(unsigned char *)(&((unsigned char *)read_in)[BS_Reserved1 + BPB_Fat32ExtByts]) = 0;
    *(unsigned char *)(&((unsigned char *)read_in)[BS_BootSig + BPB_Fat32ExtByts])   = 0x29;
    memcpy((void *)&((unsigned char *)read_in)[BS_FileSysType + BPB_Fat32ExtByts],
           (void *)"FAT32   ", 8);
    *(unsigned int *)(&((unsigned char *)read_in)[BS_VolD + BPB_Fat32ExtByts]) = rand();
    memcpy((void *)&((unsigned char *)read_in)[BS_VolLab + BPB_Fat32ExtByts], (void *)"POWERINTDOS",
           11);
    Disk_Write(0, 1, read_in);
    unsigned int *fat = (unsigned int *)_malloc(fatsz * 512);
    fat[0]            = 0xffffff0;
...
void fat_InitFS() {
  ...
  if (type != 32) {
    ...
    if (*(unsigned short *)(boot_sector + BPB_TotSec16) != 0) {
      imgTotalSize = *(unsigned short *)(boot_sector + BPB_TotSec16) * SectorBytes;
    } else {
      imgTotalSize = *(unsigned int *)(boot_sector + BPB_TotSec32) * SectorBytes;
    }
    Fat2Address = Fat1Address + *(unsigned short *)(boot_sector + BPB_FATSz16) * SectorBytes;
  } else {
    FileDataAddress = (*(unsigned char *)(boot_sector + BPB_NumFATs) *
                      ...
                      SectorBytes;
    RootDictAddress =
        FileDataAddress + (*(unsigned int *)(boot_sector + BPB_RootClus) - 2) * ClustnoBytes;
    imgTotalSize = *(unsigned int *)(boot_sector + BPB_TotSec32) * SectorBytes;
    Fat2Address  = Fat1Address + *(unsigned int *)(boot_sector + BPB_FATSz32) * SectorBytes;
  ...
  read_fat((unsigned char *)(ADR_DISKIMG + (unsigned int)Fat1Address), fat, FatClustnoFlags,
           FatMaxTerms, type);
  root_directory = (struct FAT_FILEINFO *)_malloc(RootMaxFiles * 32);
  memcpy((void *)root_directory, (void *)ADR_DISKIMG + RootDictAddress, RootMaxFiles * 32);
  ...
  for (int i = 0; i != RootMaxFiles; i++) {
    if (finfo[i].type == 0x10 && finfo[i].name[0] != 0xe5) {
      ...
      uint32_t sec1 =
          (FileDataAddress +
           (get_clustno(finfo[i].clustno_high, finfo[i].clustno_low) - 2) * ClustnoBytes) /
          SectorBytes;
  ...
  for (int i = 1; FindForCount(i, (struct List *)directory_list) != NULL; i++) {
    ...
    for (int j = 0; j != ClustnoBytes / 32; j++) {
      if (finfo[j].type == 0x10 && finfo[j].name[0] != 0xe5 &&
        ...
        uint32_t sec1 =
            (FileDataAddress +
             (get_clustno(finfo[j].clustno_high, finfo[j].clustno_low) - 2) * ClustnoBytes) /
            SectorBytes;
        Disk_Read(sec1, ClustnoBytes / SectorBytes, directory_alloc);
        AddVal((uintptr_t)directory_alloc, (struct List *)directory_list);
      }
...
bool Fat_cd(char *dictName) {
  return changedict(dictName);
}
bool Fat_ReadFile(char *path, char *buffer) {
  // printf("Fat12 Read ---- %s\n",path);
  struct FAT_FILEINFO *finfo;
  ...
  if (finfo == 0) {
    ...
    file_loadfile(get_clustno(finfo->clustno_high, finfo->clustno_low), finfo->size, buffer, fat);
    return 1;
  }
}
bool Fat_WriteFile(char *path, char *buffer, int size) {
  ...
  // printf("finfo = %08x\n",finfo);
  file_savefile(get_clustno(finfo->clustno_high, finfo->clustno_low), size, buffer, fat,
                FatClustnoFlags);
  // printf("file save ok\n");
  finfo->size = size;
  file_saveinfo(Get_dictaddr(path));
...
bool Fat_DelFile(char *path) {
  return del(path);
}
bool Fat_DelDict(char *path) {
  return deldir(path);
}
int Fat_FileSize(char *filename) {
  if (Get_File_Address(filename) == NULL) { return -1; }
...
bool Fat_CreateDict(char *filename) {
  ...
  if (finfo == root_directory) { last_clustno = 0; }
  mkdir(filename, last_clustno);
}
bool Fat_Attrib(char *filename, ftype type) {
  return attrib(filename, type);
}
...
void cmd_dir(char **args) {
  ...
  if (file != NULL) {
    printf("%s  %d  %04d-%02d-%02d %02d:%02d  ", file->name, file->size, file->year, file->month,
...
void command_run(string commands) {
  if (commands == "cls") {
    ...
    if (Fat_cd((char *)commands.c_str() + 3) == 0) { printf("Invalid directory.\n\n"); }
