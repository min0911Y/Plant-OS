...
执行程序继承调用者的工作目录和当前盘符，带盘符的可执行文件路径不隐式切盘。
解释器描述符在装载结束后关闭，原程序描述符由用户态解释器读取并关闭；
任务退出也会关闭未完成装载的所有描述符。进入解释器前的失败会报告
`exec: <路径> failed at <阶段>`，成功启动保持静默。
...
`PLANT_OPENJDK_DISK` 改名），删除 JDK 私带的系统运行库，统一使用启动盘
`/lib` 中与加载器配套的版本，并将 JDK 启动器的解释器改为 `/lib/ld.so`。
内核启动时保留 `R:` initramfs 作为系统盘，同时挂载附加块设备；
