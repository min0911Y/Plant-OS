...
`tty_get_pointer()` 通过当前 TTY 的 `TTY_RPC_POINTER` 查询最后一次窗口内指针
事件的字符坐标（列、行从 1 开始）。term 从实际客户区尺寸与行列数转换坐标，
未收到有效事件返回 -1。内核验证用户写入范围、服务身份及返回坐标范围，
调用不改变键盘或鼠标 owner。该接口供原生终端应用的平台适配使用，
例如 [RenderTM](rendertm.md)，不依赖 os-terminal 未提供的鼠标运动 API。

## 资源与容量

图层对象独立分配，显示顺序数组动态增长，像素归属保存图层指针。合成从前向后确定可见像素，完全遮住的窗口不参与后续合成，不受 8 位图层编号限制。

GUI 的事件区和两个互不重叠的像素平面由同一个 `vm_map()` 区域持有；term 的复制提交保持绘图指针稳定，整帧交换接口则交接两个平面的所有权。关闭窗口时解除映射，物理页按引用计数释放。GUI 定期检查 owner 的 TID/generation，清理异常退出客户端的窗口；其他地址空间尚持有的共享页在最后一个引用释放前不会复用。
