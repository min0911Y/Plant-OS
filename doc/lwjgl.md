...
Plant OS x86_64 已接入 LWJGL 3.3.6 的 core、GLFW、OpenGL、STB 和 OpenAL Java 绑定及对应 JNI native 库。版本、上游来源、SHA-256、许可证和补丁见 [apps/lwjgl/UPSTREAM.md](../apps/lwjgl/UPSTREAM.md) 与 [sources.json](../apps/lwjgl/sources.json)。本移植不伪装 `os.name` 为 Linux，也不加载宿主 Linux 的库。
...
- `lwjgl/jar/`：`lwjgl-3.3.6.jar`、`lwjgl-glfw-3.3.6.jar`、`lwjgl-opengl-3.3.6.jar`、`lwjgl-stb-3.3.6.jar`、`lwjgl-openal-3.3.6.jar` 及编译期的 `jspecify-1.0.0.jar`；
...
C:/java/lwjgl/jar/              五个 LWJGL JAR 和 jspecify JAR
...
三套 LWJGL native 库只依赖系统 `/lib/libp.so`。`libglfw.so` 仍来自 `apps/glfw`，并依赖 `libEGL.so` 和 `libGL.so`；不会在 LWJGL 目录中重复打包 GLFW、Mesa 或宿主 JDK native 库。

LWJGL 的 `PLANTOS` 平台通过现有 ELF `dlopen/dlsym/dlclose` 装载库和目标 JDK JNI。libffi closure 用 `vm_map` 与 `vm_map_alias` 建立 RW 数据视图和 RX 代码视图，未申请 RWX。GLFW 的 Plant runtime hook 按 `libEGL.so`、`libGL.so` 重新打开图形模块并查询其符号作用域，保留 Mesa 唯一的 EGL/GL dispatch 和 TLS。
...
| GLFW | Java GLFW 绑定、原生 `libglfw.so`、Plant GUI/EGL 后端 | 初始化、实际窗口、GLFW error/key callback、事件等待/唤醒、正常 terminate | 不支持全屏、最大化、raw motion、剪贴板、IME、GLFW Vulkan WSI；完整边界见 [GLFW](glfw.md) |
...
生成绑定时，GLFW 的 `GLFWNativeEGL` 和 `GLFWVulkan` 被按当前 Plant 能力排除；IME/preedit 符号在现有 GLFW 3.4 中按 optional lookup 处理，但 Plant 当前没有 IME 实现。Vulkan、其他可选 LWJGL 模块及 natives JAR 自动提取仍未接入。Minecraft 的
应用依赖版本与启动说明见 [客户端](minecraft-client.md)。

OpenAL Java 绑定通过 core JNI 调用系统 `libopenal.so`，不需要额外的
`liblwjgl_openal.so`。原生 OpenAL Soft 提供无输出设备与 loopback；
冒烟检查真实 PCM 混音、上下文和资源释放，没有声卡输出。构建来源见
[OpenAL](../apps/openal/UPSTREAM.md)。
...
脚本通过 QMP 只投递键盘事件，不注入客户机命令；它轮询真实截图，检测红色初帧后发送 A，检测绿色终帧后保存第二帧。Java 将 `JNI`、`CORE`、`CALLBACK`、`STB`、`GLFW`、`OPENGL`、`OPENAL` 阶段写入 `C:/java/lwjgl/lwjgl-result.txt`，正常异常路径同时写出 `lwjgl-failure.txt`；脚本在 FAT 盘取回这些文件并尝试取回 JVM fatal 日志，再检查 Java 命令状态、ACPI S5 和两帧像素不同。

当前七阶段回归已在 BIOS/KVM 下通过，包括两帧像素、真实 PCM 混音、Java 正常退出和 S5。完整 LWJGL 上游测试矩阵尚未执行。
