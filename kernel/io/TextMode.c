#include <dos.h>
void move_cursor_by_idx(int count);
...
void putchar_TextMode(struct tty *res, int c) {
  ...
  if (c == '\n') {
    ...
    if (res->x == 0) {
      ...
      *(u8 *)(res->vram + res->y * res->xsize * 2 + res->x * 2)         = ' ';
      *(u8 *)(res->vram + res->y * res->xsize * 2 + res->x * 2 - 2 + 1) = res->color;
      return;
    }
    *(u8 *)(res->vram + res->y * res->xsize * 2 + res->x * 2 - 2)     = ' ';
    *(u8 *)(res->vram + res->y * res->xsize * 2 + res->x * 2 - 2 + 1) = res->color;
  ...
  *(u8 *)(res->vram + res->y * res->xsize * 2 + res->x * 2)     = c;
  *(u8 *)(res->vram + res->y * res->xsize * 2 + res->x * 2 + 1) = res->color;
  res->MoveCursor(res, res->x + 1, res->y);
}
...
void screen_ne_TextMode(struct tty *res) {
  for (int i = 0; i < res->xsize * 2; i += 2) {
    for (int j = 0; j < res->ysize; j++) {
      *(u8 *)(res->vram + j * res->xsize * 2 + i) =
          *(u8 *)(res->vram + (j + 1) * res->xsize * 2 + i);
      *(u8 *)(res->vram + j * res->xsize * 2 + i + 1) =
          *(u8 *)(res->vram + (j + 1) * res->xsize * 2 + i + 1);
    }
  }
  for (int i = 0; i < res->xsize * 2; i += 2) {
    *(u8 *)(res->vram + (res->ysize - 1) * res->xsize * 2 + i)     = ' ';
    *(u8 *)(res->vram + (res->ysize - 1) * res->xsize * 2 + i + 1) = res->color;
  }
...
void clear_TextMode(struct tty *res) {
  for (int i = 0; i < res->xsize * 2; i += 2) {
    for (int j = 0; j < res->ysize; j++) {
      *(u8 *)(res->vram + j * res->xsize * 2 + i)     = ' ';
      *(u8 *)(res->vram + j * res->xsize * 2 + i + 1) = res->color;
    }
  }
  res->gotoxy(res, 0, 0);
  res->Raw_y = 0;
