...
static struct ACPISDTHeader *acpi_root;
...
unsigned int* acpi_find_rsdp(void) {
  ...
  uint8_t *end = start + 0x00020000u;
  for (uint8_t *addr = start; addr + 20 <= end; addr += 16) {
    if (memcmp(addr, "RSD PTR ", 8) == 0) {
      struct ACPI_RSDP *candidate = (struct ACPI_RSDP *)addr;
      bool valid = checksum(addr, 20);
      if (valid && candidate->Revision >= 2) {
        valid = candidate->Length >= sizeof(*candidate) &&
                candidate->Length <= (uint32_t)(end - addr) &&
                checksum(addr, candidate->Length);
      }
      if (valid) {
...
void* acpi_find_table(char* Signature) {
  if (acpi_root == NULL || acpi_root->Length < sizeof(*acpi_root)) {
    return NULL;
  }
  uint8_t *entries = (uint8_t *)acpi_root + sizeof(*acpi_root);
  size_t entry_size = acpi_root_xsdt ? sizeof(uint64_t) : sizeof(uint32_t);
  size_t count = (acpi_root->Length - sizeof(*acpi_root)) / entry_size;
  for (size_t index = 0; index < count; index++) {
    uint64_t address = 0;
    memcpy(&address, entries + index * entry_size, entry_size);
    struct ACPISDTHeader *header =
        arch_mmio_map(address, sizeof(struct ACPISDTHeader));
    if (header == NULL || header->Length < sizeof(*header)) {
      continue;
    }
    header = arch_mmio_map(address, header->Length);
    if (header != NULL && checksum((unsigned char *)header, header->Length) &&
        memcmp(header->Signature, Signature, 4) == 0) {
      return header;
    }
  }
  return NULL;
...
static void hpet_initialize(void) {
  ...
  hpetInfo = arch_mmio_map(address, sizeof(HpetInfo));
...
void init_acpi(void) {
  ...
  uint64_t xsdt_address = (uint64_t)RSDP->XsdtAddress[0] |
                          (uint64_t)RSDP->XsdtAddress[1] << 32;
  uint64_t root_address =
      RSDP->Revision >= 2 && xsdt_address != 0 ? xsdt_address
                                              : RSDP->RsdtAddress;
  acpi_root_xsdt = RSDP->Revision >= 2 && xsdt_address != 0;
  acpi_root = arch_mmio_map(root_address, sizeof(*acpi_root));
  if (acpi_root == NULL || acpi_root->Length < sizeof(*acpi_root)) {
    logk("acpi: root table cannot be mapped\n");
    return;
  }
  acpi_root = arch_mmio_map(root_address, acpi_root->Length);
  if (acpi_root == NULL ||
      !checksum((unsigned char *)acpi_root, acpi_root->Length)) {
    logk("acpi: invalid root table checksum\n");
    return;
  }
