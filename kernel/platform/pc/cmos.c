...
unsigned char read_cmos(unsigned char p) {
  unsigned char data;
  x86_port_write8(CMOS_INDEX_PORT, p);
  data = x86_port_read8(CMOS_DATA_PORT);
  x86_port_write8(CMOS_INDEX_PORT, 0x80);
  return data;
}
void write_cmos(unsigned char p, unsigned char data) {
  x86_port_write8(CMOS_INDEX_PORT, p);
  x86_port_write8(CMOS_DATA_PORT, data);
  x86_port_write8(CMOS_INDEX_PORT, 0x80);
  x86_port_write8(CMOS_DATA_PORT, 0);
}
