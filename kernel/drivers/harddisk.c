...
void bios_read_hd_sec(unsigned LBA, unsigned char* buffer) {
  ...
  memcpy((void *)0x7e00, &p, sizeof(packet));
...
void drivers_idehdd_read(unsigned int LBA,
  ...
  // x86_port_write8(0x1f7, 0x0c);
  // x86_port_write16(0x1f2, number);              /**数量*/
  // x86_port_write8(0x1f3, (LBA & 0xff));         /**LBA地址7~0*/
  // x86_port_write8(0x1f4, ((LBA >> 8) & 0xff));  /**LBA地址15~8*/
  // x86_port_write8(0x1f5, ((LBA >> 16) & 0xff)); /**LBA地址23~16*/
  // x86_port_write8(0x1f6,
  //         (((LBA >> 24) & 0xff) | 0xe0)); /**LBA地址27~24 | LBA模式，主硬盘*/
  // x86_port_write8(0x1f7, 0x20);                   /**读扇区*/
  // /**循环从DATA端口读取数据*/
  // for (i = 0; i != number; i++) {
  //   drivers_idehdd_wait();
  //   for (offset = 0; offset < 256; offset++) {
  //     buffer[(i * 256) + offset] = x86_port_read16(0x1f0); /**从DATA端口中读取数据*/
  //   }
  // }
  // /**开放任务切换*/
...
void drivers_idehdd_wait1(void) {
  for (; (x86_port_read8(0x1f7) & 0x80) == 0x80;)
    ; /**循环等待*/
}
void drivers_idehdd_wait2(void) {
  for (; (x86_port_read8(0x1f7) & 0x08) != 0x08;)
    ; /**循环等待*/
}
void drivers_idehdd_wait(void) {
  /**等待次数计时*/
  // unsigned long n;
  for (; (x86_port_read8(0x1f7) & 0x88) != 0x08;)
    ; /**循环等待*/
}
...
struct IDEHardDiskInfomationBlock* drivers_idehdd_info() {
  ...
  x86_port_write8(0x1f7, 0xec);
  for (int i = 0; i != 256; i++) {
    p[i] = x86_port_read16(0x1f0); /**从DATA端口中读取数据*/
  }
  irq_restore(state);
