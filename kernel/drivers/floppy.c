...
static volatile int floppy_io_failed = 0;
...
void init_floppy() {
  ...
  if (floppy_io_failed) {
    printk("floppy: controller did not respond\n");
    return;
  }
  sendbyte(CMD_VERSION);               // 获取软盘版本
  printk("FDC_VER:0x%x\n", getbyte()); // 并且输出到屏幕上

  /* The controller responds even when the drive has no medium.  Probe one
   * sector before publishing the disk so filesystem probing cannot block on
   * an empty drive. */
  unsigned char probe[512];
  floppy_io_failed = 0;
  if (!fdc_rw(0, probe, 1, 1) || floppy_io_failed) {
    printk("floppy: no media\n");
    return;
  }
...
void bios_fdc_rw(int block, unsigned char *blockbuff, int read,
  ...
  unsigned char *p_tbaddr = (unsigned char *)0x7e00; // 512byte
  (void)nosectors;
  /* 获取block对应的ths */
  block2hts(block, &track, &head, &sector);
  if (!read) {
    memcpy(p_tbaddr, blockbuff, 512);
  }
  regs16_t r;
