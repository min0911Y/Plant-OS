...
static void print_ipv4_value(const char* name, uint32_t value) {
  printk("%s: %d.%d.%d.%d\n", name, (uint8_t)(value >> 24),
...
void init_card() {
  ...
  for (int i = 0; i < 25; i++) {
    ...
    if (network_card_CTL[i].find()) {
      ...
      extern uint32_t ip, gateway, submask, dns, dhcp_ip;
      ...
      printk("network: %s ready\n", network_card_CTL[i].card_name);
      print_ipv4_value("network dhcp", dhcp_ip);
      print_ipv4_value("network ip", ip);
      print_ipv4_value("network gateway", gateway);
      print_ipv4_value("network subnet", submask);
      print_ipv4_value("network dns", dns);
...
void netcard_send(unsigned char* buffer, unsigned int size) {
  static unsigned char netcard_drive_name[] = "NETCARD_DRIVE";
  for (int i = 0; i < 25; i++) {
    if (network_card_CTL[i].use) {
      if (DriveSemaphoreTake(GetDriveCode(netcard_drive_name))) {
        // printk("Send....%s %d
        // %d\n",network_card_CTL[i].card_name,network_card_CTL[i].use,i);
        network_card_CTL[i].Send(buffer, size);
        DriveSemaphoreGive(GetDriveCode(netcard_drive_name));
        break;
      }
    }
  }
