...
uint8_t *ICMP_Packet(uint8_t type, uint8_t code, uint16_t ID, uint16_t sequence, uint8_t *data,
                     uint32_t size) {
...
void ICMPProviderSend(uint32_t destip, uint32_t srcip, uint8_t type, uint8_t code, uint16_t ID,
                      uint16_t sequence, uint8_t *data, uint32_t size) {
...
int ping(uint32_t dstIP) {
  ...
  uint8_t *dat = ping_reply_data + sizeof(struct EthernetFrame_head) + sizeof(struct IPV4Message) +
...
void icmp_handler(void *base) {
  ...
  if (icmp->type == 8 && icmp->code == 0) { // Ping请求
    uint32_t size  = (((ipv4->totalLength & 0xff00) >> 8) | ((ipv4->totalLength & 0x00ff) << 8));
    size          -= sizeof(struct IPV4Message);
    size          -= sizeof(struct ICMPMessage);
    uint8_t *data  = base + sizeof(struct EthernetFrame_head) + sizeof(struct IPV4Message) +
                    sizeof(struct ICMPMessage);
    uint32_t src_ip = ((ipv4->srcIP << 24) & 0xff000000) | ((ipv4->srcIP << 8) & 0x00ff0000) |
                      ((ipv4->srcIP >> 8) & 0xff00) | ((ipv4->srcIP >> 24) & 0xff);
    uint8_t *packet = ICMP_Packet(0, 0, swap16(icmp->ID), swap16(icmp->sequence), data, size);
