#include <dos.h>
static u8             chat_data[2048]; // 接收网卡发来的数据包
static u8             chat_flags = 1;  // 网卡发来数据包了吗？
...
static void out_put_task_main_cmd() { // 这是一个关于接收数据并输出的进程
  while (1) {
    ...
    u8 *dat = chat_data + sizeof(struct EthernetFrame_head) + sizeof(struct IPV4Message) +
...
void chat_cmd() { // 用于接收用户想要发送的内容
  // extern u32 ip; // 系统IP地址
  // /* 打印提示符 */
  // printk("Welcome to Powerint DOS 386 Chat!\n");
  // printk("Connect 118.31.248.215...\n");
  // if (ping(CHAT_SERVER_IP) == -1) { // 网卡正常撒？
  //   printk("The ICMP packet is not from 118.31.248.215.\n"); // 连接失败
  //   return;
  // }
  // printk("Connect 118.31.248.215 done.\n"); // 到这里说明连接成功
  // printk("Connect Server...\n"); // 现在通过UDP协议连接服务器
  ...
  // clean((char *)chat_data, 2048); // 清空缓冲区
  // socket->Send(socket, (u8 *)"CONNECT ", strlen("CONNECT ")); // 告诉服务器我们已经连接
  // while (chat_flags) // 等待服务器回复
  //   ;
  // u8 *dat = chat_data + sizeof(struct EthernetFrame_head) +
  //                sizeof(struct IPV4Message) + sizeof(struct UDPMessage); // 获取数据
  // if (strcmp((char *)dat, "OK") != 0) { // 如果服务器是chat程序的正确服务端，那么将会返回OK字样
  //   printk("The UDP packet is not from 118.31.248.215.\n"); // 啊哦，不是
  //   return;
  // }
  // printk("Connect Server done.\n"); // 到这里说明是了
  ...
  // (void)(out_put_task);
  // while (true) {
  //   printk("To 118.31.248.215 Server ### "); // 提示符信息
  //   input(&inp[5], 1019); // 输入，&inp[5]是因为上面声明了指令（五个字节），同样，1019是通过1024-5得到的
  //   socket->Send(socket, (u8 *)inp, strlen(inp)); // 通过socket API发送
  // }
}
/* 声明用到的GUI控件 */
...
void chat_gui() {
  ...
  clean((char *)chat_data, 2048);
  socket->Send(socket, (u8 *)"CONNECT ", strlen("CONNECT "));
  while (chat_flags)
    ;
  u8 *dat = chat_data + sizeof(struct EthernetFrame_head) +
                 sizeof(struct IPV4Message) + sizeof(struct UDPMessage);
