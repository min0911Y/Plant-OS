...
#define X86_IRQ_VECTOR_BASE 0x20u
#define APIC_TIMER_VECTOR X86_IRQ_VECTOR_BASE
...
static void ioapic_init_from_acpi(void) {
  ...
  for (uint32_t i = 0; i < ioapic_total; i++) {
    ...
    ioapics[i].virt = arch_mmio_map(ioapics[i].phys, 0x20);
    if (ioapics[i].virt == NULL) {
      ioapic_total = i;
      break;
    }
  ...
  for (uint32_t irq = 0; irq < MAX_IRQS; irq++) {
    ioapic_program_gsi(irq_to_gsi[irq], X86_IRQ_VECTOR_BASE + irq, 1,
                       irq_trigger[irq], irq_polarity[irq]);
  }
}

void init_pic(void) {
  x86_port_write8(X86_PIC0_DATA, 0xff);
  x86_io_wait();
  x86_port_write8(X86_PIC1_DATA, 0xff);
  x86_io_wait();
  x86_port_write8(X86_PIC0_COMMAND, 0x11);
  x86_io_wait();
  x86_port_write8(X86_PIC0_DATA, X86_IRQ_VECTOR_BASE);
  x86_io_wait();
  x86_port_write8(X86_PIC0_DATA, 1 << 2);
  x86_io_wait();
  x86_port_write8(X86_PIC0_DATA, 0x01);
  x86_io_wait();

  x86_port_write8(X86_PIC1_COMMAND, 0x11);
  x86_io_wait();
  x86_port_write8(X86_PIC1_DATA, X86_IRQ_VECTOR_BASE + 8);
  x86_io_wait();
  x86_port_write8(X86_PIC1_DATA, 2);
  x86_io_wait();
  x86_port_write8(X86_PIC1_DATA, 0x01);
  x86_io_wait();

  x86_port_write8(X86_PIC0_DATA, pic_irq_masks[0]);
  x86_port_write8(X86_PIC1_DATA, pic_irq_masks[1]);
}
...
void send_eoi(int irq) {
  ...
  if (irq >= 8) {
    x86_port_write8(X86_PIC1_COMMAND, 0x60 | (irq - 8));
    x86_port_write8(X86_PIC0_COMMAND, 0x60 | 2);
  } else {
    x86_port_write8(X86_PIC0_COMMAND, 0x60 | irq);
  }
}
...
void apic_configure_irq(unsigned irq, int trigger_mode, int polarity) {
  ...
  ioapic_program_gsi(irq_to_gsi[irq], X86_IRQ_VECTOR_BASE + irq, irq_masked[irq],
...
void apic_unmask_irq(unsigned irq) {
  ...
  irq_masked[irq] = 0;
  ioapic_program_gsi(irq_to_gsi[irq], X86_IRQ_VECTOR_BASE + irq, 0,
                     irq_trigger[irq], irq_polarity[irq]);
}
