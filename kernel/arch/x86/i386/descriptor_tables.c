...
static void x86_interrupt_entry_set(unsigned vector,
                                    x86_interrupt_entry_t entry,
                                    uint16_t access) {
...
void arch_interrupt_init(void) {
  ...
  if (bios_ivt == NULL) {
    bios_ivt = page_malloc(0x400);
    if (bios_ivt == NULL) {
      x86_boot_fail("plant os: unable to preserve BIOS interrupt vectors");
    }
    memcpy(bios_ivt, (const void *)0, 0x400);
  }
...
void x86_user_frame_init(x86_interrupt_frame_t *frame, uint32_t eip,
  ...
  frame->ecx = 7;
  frame->eax = 8;
  frame->gs = X86_USER_SELECTOR(X86_GDT_USER_DATA_INDEX);
...
void x86_bios_interrupt(uint8_t interrupt_number, regs16_t *registers) {
  ...
  x86_cr3_write(current_task()->address_space);
  irq_restore(state);
}
