...
static boot_module_t boot_initramfs;
...
int x86_boot_prepare(uint32_t magic, uintptr_t information_address) {
  ...
  boot_initramfs.address = destination;
  boot_initramfs.size = module_size;
  return 0;
}

bool arch_boot_initramfs(boot_module_t *module) {
  if (module == NULL || boot_initramfs.size == 0) {
    return false;
  }
  *module = boot_initramfs;
  return true;
}
