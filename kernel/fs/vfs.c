...
bool vfs_clone_for_task(mtask *src, mtask *dest) {
  ...
  if (!vfs_change_disk_for_task(src->nfs->drive, dest)) {
    return false;
  }
  List *l;
  char *path;
